...
class _DeliveryModePageState extends State<DeliveryModePage> {
  ...
  void _fetchDeliveries({bool showLoader = true}) {
    ...
    if (authState is AuthAuthenticated && authState.storeId != null) {
      context.read<StaffDeliveryBloc>().add(FetchAssignedDeliveriesEvent(
      ...
      body: BlocConsumer<StaffDeliveryBloc, StaffDeliveryState>(
        ...
        builder: (context, state) {
          ...
          if (state is StaffDeliveryLoaded) {
            ...
            // KPI Calculations
            final codOrders = deliveries.where((o) => o.payment.method == 'cash').toList();
            final totalCODAmount = codOrders.fold<int>(0, (sum, o) => sum + o.totalPrice);

            return Column(
              children: [
                // KPI Metrics Banner
                _buildKpiBanner(deliveries.length, codOrders.length, totalCODAmount),
                ...
                Expanded(
                  child: deliveries.isEmpty
                      ? const EmptyStateWidget(
                          ...
                          subtitle: 'Bạn đã hoàn thành xuất sắc tất cả chuyến giao hàng!',
                        )
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final isActioning = state.actioningOrderId == order.id;

                            return _buildDeliveryCard(context, order, storeId, driverId, isActioning);
                          },
                        ),
                ),
              ],
  ...
  Widget _buildDeliveryCard(
    ...
    final customerName = order.customer?.fullName ?? order.customer?.username ?? 'Khách vãng lai';
    final customerPhone = order.customer?.phone ?? order.deliveryAddress.phone;
    final fullAddress = '${order.deliveryAddress.detail}, ${order.deliveryAddress.ward}, ${order.deliveryAddress.city}';
        ...
        child: Column(
          ...
          children: [
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: isCOD ? Colors.orange.shade100 : Colors.green.shade100),
                      ),
                      child: Text(
                        isCOD ? 'COD' : 'ONLINE',
                        style: TextStyle(
            ...
            InkWell(
              ...
              child: Container(
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Icon(Icons.map_outlined, color: AppColors.textSecondary, size: 16),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
            ...
            isActioning
                ...
                : ElevatedButton(
                    onPressed: () => _showConfirmationModal(context, order, storeId, driverId),
                    style: ElevatedButton.styleFrom(
                      minimumSize: const Size(double.infinity, 46),
                      backgroundColor: isCOD ? Colors.orange.shade700 : Colors.green.shade700,
                    ),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text(isCOD ? 'ĐÃ THU COD & HOÀN THÀNH' : 'XÁC NHẬN ĐÃ GIAO'),
                      ],
                    ),
                  ),
          ],
  ...
  void _showConfirmationModal(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            Icon(Icons.local_shipping, color: isCOD ? Colors.orange : Colors.green),
            const SizedBox(width: 8),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    const Text(
                      'SỐ TIỀN CẦN THU COD',
                      style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: Colors.orange),
                    ),
                    const SizedBox(height: 4),
                    Text(
                      Formatters.currency(order.totalPrice),
                      style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.orange.shade900),
                    ),
                  ],
                ),
              )
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              context.read<StaffDeliveryBloc>().add(CompleteDeliveryEvent(
                    orderId: order.id,
                    storeId: storeId,
                    driverId: driverId,
                  ));
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Chi tiết đơn #${order.code}', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18)),
                      ...
                      child: Row(
                        children: [
                          Container(
                            ...
                            child: Text('x${item.quantity}', style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary, fontSize: 13)),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(item.name ?? item.product?.name ?? 'Sản phẩm', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                          ...
                          Text(Formatters.currency(item.subTotal), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                        ],
                      ),
                    );
                  },
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Tổng cộng cần thu', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                  PriceText(price: order.totalPrice, fontSize: 22, fontWeight: FontWeight.w900),
                ],
              ),
              const SizedBox(height: 16),
            ],
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            label,
            style: TextStyle(fontSize: 8, fontWeight: FontWeight.w900, color: color.withOpacity(0.7), letterSpacing: 0.5),
            textAlign: TextAlign.center,
          ),
        ],
      ),
