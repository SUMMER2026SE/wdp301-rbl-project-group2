...
class _AddressPageState extends State<AddressPage> {
  ...
  Future<void> _loadAddresses() async {
    ...
    try {
      ...
      setState(() {
        _error =
            e.response?.data['message'] as String? ?? 'Không thể tải địa chỉ';
        _loading = false;
      });
    } catch (_) {
      if (!mounted) return;
  ...
  Map<String, dynamic> _normalizeAddress(Map<String, dynamic> source) {
    return {
      ...
      'receiverName':
          source['receiverName'] as String? ?? source['name'] as String? ?? '',
      'phone': (source['phone'] as String? ?? '').replaceAll(
        RegExp(r'\s+'),
        '',
      ),
      'detail':
          source['detail'] as String? ?? source['address'] as String? ?? '',
  ...
  Future<void> _deleteAddress(String id) async {
    ...
    final filtered = previous.where((e) {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Không thể xoá địa chỉ')));
      }
    }
  }
  ...
  Future<void> _setDefault(String id) async {
    try {
      final updated = _normalizeAddressList(_addresses).map((m) {
        final itemId =
            m['_id'] as String? ?? m['id'] as String? ?? m.hashCode.toString();
        return {...m, 'isDefault': itemId == id || m.hashCode.toString() == id};
      }).toList();
      await _saveAddresses(updated);
      if (!mounted) return;
  ...
  void _showAddressForm({Map<String, dynamic>? existing}) {
    ...
    final nameCtrl = TextEditingController(
      text:
          existing?['receiverName'] as String? ??
  ...
  Widget _buildBody() {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, i) =>
            _buildAddressCard(_addresses[i] as Map<String, dynamic>),
      ),
    );
  }
  ...
  Widget _buildAddressCard(Map<String, dynamic> addr) {
    ...
    final name =
        addr['receiverName'] as String? ?? addr['name'] as String? ?? '';
    final phone = addr['phone'] as String? ?? '';
    final detail =
        addr['detail'] as String? ?? addr['address'] as String? ?? '';
