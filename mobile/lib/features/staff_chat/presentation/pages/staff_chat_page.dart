...
class _StaffChatPageState extends State<StaffChatPage> {
  ...
  void _fetchMessages() {
    context.read<StaffChatBloc>().add(
      FetchMessagesEvent(conversationId: widget.conversationId),
    );
  }
  ...
  void _setupSocketRoom() {
    ...
    SocketService().on('support:new_message', (data) {
      if (data != null) {
        final rawMessage = data is Map && data['data'] != null
            ? data['data']
            : data;
        if (rawMessage is! Map<String, dynamic>) return;
        final newMsg = ChatMessageModel.fromJson(rawMessage);
        if (mounted) {
          context.read<StaffChatBloc>().add(
            ReceiveRealtimeMessageEvent(newMsg),
          );
          _scrollToBottom();
        }
      }
  ...
  void _fetchOrderDetail(String orderId) async {
    ...
    if (authState is AuthAuthenticated && authState.storeId != null) {
      ...
      final result = await _getStaffOrderByIdUseCase(
        orderId: orderId,
  ...
  void dispose() {
    ...
    SocketService().off('support:new_message');
    super.dispose();
  }
  ...
  void _sendQuickReply(String reply) {
    context.read<StaffChatBloc>().add(
      SendChatTextEvent(conversationId: widget.conversationId, content: reply),
    );
    _scrollToBottom();
  }
  ...
  void _closeChat() {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Bạn có chắc chắn muốn đóng cuộc trò chuyện hỗ trợ này?',
        ),
        actions: [
          TextButton(
