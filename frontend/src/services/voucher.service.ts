...
class VoucherAPI {
  ...
  async getVoucherById(id: string): Promise<VoucherDetailResponse> {
    const response = await apiClient.get<VoucherDetailResponse>(
      `${this.baseURL}/${id}`,
    );
    return response.data;
  }
  ...
  async createVoucher(
    ...
    const response = await apiClient.post<VoucherDetailResponse>(
      this.baseURL,
      data,
    );
    return response.data;
  ...
  async updateVoucher(
    id: string,
    data: Partial<CreateVoucherRequest>,
  ): Promise<VoucherDetailResponse> {
    const response = await apiClient.put<VoucherDetailResponse>(
      `${this.baseURL}/${id}`,
      data,
    );
    return response.data;
