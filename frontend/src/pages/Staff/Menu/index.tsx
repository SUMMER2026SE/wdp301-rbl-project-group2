...
export default function StaffMenu() {
  ...
  const fetchMenuData = useCallback(async (showLoader = false) => {
    ...
    try {
      ...
      const [productsRes, categoriesRes] = await Promise.all([
        ...
        }), // Staff needs to see all products including suspended from their store
  ...
  const toggleStock = async (id: string, currentActive: boolean) => {
    if (actioningId) return;
    setActioningId(id);
    const toastId = toast.loading("Đang cập nhật trạng thái món ăn...");
    try {
      const nextActive = !currentActive;
      const res = await productService.updateProductAvailability(id, {
        isAvailable: nextActive,
        status: nextActive ? "active" : "inactive",
      });
      if (res.success) {
        toast.success(
          `Đã ${nextActive ? "mở bán lại" : "tạm ngưng"} món ăn thành công!`,
          { id: toastId }
        );
        // Update local state
        setProducts((prev) =>
          prev.map((item) =>
            item._id === id
              ? { ...item, isAvailable: nextActive, status: nextActive ? "active" : "inactive" }
              : item
          )
        );
      } else {
  ...
  const filteredItems = useMemo(() => {
    return products.filter((item) => {
      ...
      const matchesStatus =
        statusFilter === "all" ||
        (statusFilter === "available" && item.isAvailable && item.status === "active") ||
        (statusFilter === "suspended" && (!item.isAvailable || item.status !== "active"));
  ...
  const inStockItems = products.filter((item) => item.isAvailable && item.status === "active").length;
          ...
          {viewMode === "grid" ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mb-8">
              {filteredItems.map((item) => {
                const isActive = item.isAvailable && item.status === "active";
                return (
                  <div
                    key={item._id}
                    ...
                    <div className="p-4 flex-1 flex flex-col">
                      ...
                      <div className="flex justify-between items-center pt-3 border-t border-slate-100">
                        ...
                        <button
                          ...
                          onClick={() => toggleStock(item._id, isActive)}
                        >
